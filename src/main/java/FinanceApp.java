...
public class FinanceApp {
  ...
  private void handleTransfer() {
    try {
      ...
      User toUser = userService.getUser(toUsername);
      if (toUser == null) {
  ...
  private void handleExpensesByCategories() {
    ...
    double sum =
        financeService.getExpensesByCategories(userService.getCurrentUser(), List.of(categories));
  ...
  private void showStatistics() {
    ...
    if (!incomeByCategory.isEmpty()) {
      ...
      incomeByCategory.forEach((category, amount) -> System.out.println(category + " : " + amount));
    }
  ...
  private void handleTransactionsByPeriod() {
    try {
      ...
      List<Transaction> list =
          financeService.getTransactionsByPeriod(userService.getCurrentUser(), from, to);
      if (list.isEmpty()) {
  ...
  private void exportStatisticsToFile() {
    ...
    try (PrintWriter writer = new PrintWriter(fileName)) {
      ...
      financeService
          .getIncomeByCategory(user)
          .forEach((cat, amount) -> writer.println(cat + " : " + amount));
